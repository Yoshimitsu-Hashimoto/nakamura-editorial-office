...
#### 1. 重複ACFフィルターの統合
- 3つの重複していたACFフィルター関数を2つに統合
- `nakamura_set_acf_default_values`: 通常フィールドとグループフィールド両方に対応
- `nakamura_prepare_field_defaults`: フィールド表示時のデフォルト値確保

#### 2. コードの簡素化
- 不要な`Contact Form 7`対応関数を削除（実処理がないため）
- 全フィールドから`instructions`記述を削除（管理画面の簡素化）

#### 3. セキュリティ設定の最適化
- REST API完全無効化を緩和（プラグイン互換性向上）
- JSONP無効化のみ維持（セキュリティ確保）
...
### 現在の実装
- フロントページのメタタグはハードコーディングで対応
- 基本的なOGタグ、Twitterカード、構造化データを実装

### 将来のSEOプラグイン導入時の対応
1. **削除対象**: `nakamura_seo_meta_tags()`関数全体を削除
2. **理由**: プラグインとの重複・競合を避けるため
3. **手順**:
   ```php
   // 以下の関数とフックを削除
...
### カスタムフィールド
- リピーターフィールドは使用しない（要件）
- グループフィールドを活用したレイアウト管理
- デフォルト値は必ず設定する

### SEO
- プラグイン導入前：現在のハードコーディング使用
- プラグイン導入後：該当関数を削除して完全にプラグインに委任